...
struct VoiceTranscriberView: View {
    ...
    var body: some View {
        NavigationView {
            ...
            .navigationTitle(aorusL("Транскриптор", "Transcriber"))
            .navigationBarTitleDisplayMode(.inline)
            .toolbar {
                ToolbarItem(placement: .cancellationAction) {
                    Button(aorusL("Закрыть", "Close")) {
    ...
    private var recordCard: some View {
        GlassCard {
            VStack(spacing: 12) {
                ...
                Text(isRecording ? aorusL("Запись... Говорите", "Recording… Speak now") : aorusL("Нажмите чтобы записать", "Tap to record"))
    ...
    private var recordButton: some View {
        ...
        return Button {
            ...
            Text(isRecording ? aorusL("Остановить", "Stop") : aorusL("Записать", "Record"))
    ...
    private var filePickerButton: some View {
        GlassButton(title: aorusL("Выбрать аудио файл", "Choose an audio file"), icon: "doc.fill", color: Color(hex: "#5C6BC0")) {
            showFilePicker = true
        }
    ...
    private var transcribingIndicator: some View {
        HStack {
            ...
            Text(aorusL("Распознавание...", "Recognizing…"))
                .font(.system(size: 13))
                .foregroundColor(.secondary)
        }
    }
        ...
        SFSpeechRecognizer.requestAuthorization { status in
            DispatchQueue.main.async {
                guard status == .authorized else {
                    errorMessage = aorusL("Нет разрешения на распознавание речи. Включите в Настройках.", "Speech recognition is not permitted. Enable it in Settings.")
                    showError = true
                    return
                }
                AVAudioSession.sharedInstance().requestRecordPermission { granted in
                    DispatchQueue.main.async {
                        guard granted else {
                            errorMessage = aorusL("Нет разрешения на микрофон", "Microphone access is not permitted")
                            showError = true
                            return
                        }
                        beginAudio()
    ...
    private func beginAudio() {
        do {
            ...
            errorMessage = aorusL("Не удалось активировать аудиосессию: %@", "Could not start the audio session: %@").replacingOccurrences(of: "%@", with: error.localizedDescription)
            showError = true
            return
        }
        ...
        guard let recognizer = SFSpeechRecognizer(locale: locale), recognizer.isAvailable else {
            errorMessage = aorusL("Распознаватель для языка %@ недоступен", "No recognizer is available for %@").replacingOccurrences(of: "%@", with: selectedLocale)
            showError = true
            return
        }
        ...
        do {
            ...
            errorMessage = aorusL("Не удалось запустить запись: %@", "Could not start recording: %@").replacingOccurrences(of: "%@", with: error.localizedDescription)
            showError = true
        }
    }
