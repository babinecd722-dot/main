...
Core Animation в отдельном окне, которое не перехватывает касания, — сквозь эффект можно
продолжать пользоваться приложением.

```js
// Идёт, пока не остановят, или заданное время. id — это ручка, за которую плагин его держит.
await aorus.effects.start('winter', 'snow', { intensity: 1.5, wind: 0.4, color: '#CFE8FF' });
await aorus.effects.stop('winter');

// Разовые: приложение убирает их само.
await aorus.effects.burst('confetti', { x: 0.5, y: 0.4, colors: aorus.color.palette('5B4DFF', 6) });
await aorus.effects.celebrate();               // фейерверк
...
| Эффект | Что это |
|---|---|
| `snow` | Падающий снег, в два слоя глубины |
| `rain` | Дождь косыми штрихами |
| `confetti` | Падающие бумажки, вращаются |
| `fireworks` | Залпы искр, сами по очереди |
| `hearts` | Сердечки снизу вверх |
| `bubbles` | Пузыри вверх |
| `sparkles` | Искры по всему экрану |
| `leaves` | Падающие листья, кружатся |
| `warp` | Звёзды из центра, как разгон |
| `emoji` | Ваши эмодзи падают или, с `rising: true`, летят вверх |

Общие параметры: `intensity` (0.1–3, сколько частиц), `speed` (0.25–3), `size` (0.25–3),
`wind` (−1…1, снос вбок), `color` или `colors` (массив «RRGGBB»), `emoji` (для `emoji` и
`leaves`), `x` и `y` (для `burst`, `ripple` — доля ширины и высоты экрана), `duration` (мс;
для `start` ноль значит «пока не остановят», иначе от секунды до десяти минут). Числа
ограничиваются, а не отклоняются: `intensity: 99` станет максимумом.

`start`, `burst`, `flash`, `shake`, `ripple` и `glow` возвращают `{ ok, shown }`. `shown`
может быть `false` — и это не ошибка, а факт: приложение решает, показывать ли эффект, и
говорит почему в поле `reason`.

| `reason` | Когда |
|---|---|
| `reduceMotion` | Включено «Уменьшение движения» — движущиеся эффекты пропускаются, вспышка приглушается, свечение показывается |
| `thermal` | Телефон перегрет — новые движущиеся эффекты не запускаются |
| `background` | Приложение не на экране |
...
Правила, которые эффект соблюдает всегда: не перехватывает касания, учитывает «Уменьшение
движения», не мигает чаще безопасного порога, при экономии заряда рисует меньше частиц, а
при перегреве телефона не запускает новых. Всё, что плагин нарисовал, исчезает, когда плагин
останавливается. До трёх непрерывных эффектов на плагин. Требует `screenEffects` — отдельное
разрешение, потому что это не собственный экран плагина, а анимация поверх чужого.
