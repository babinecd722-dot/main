...
Эффект появляется сразу после `start` — при включении плагина, после сохранения кода или по
...
Повторный `start` с тем же `id` не начинает эффект заново: новые параметры подхватываются, а
частицы, которые уже летят, долетают до края. `stop` прекращает появление новых частиц; те, что
уже на экране, уходят сами.

Когда плагин перезапускается — после сохранения кода или при смене аккаунта, — `stop` старого
...
его `duration`, телефон перегрелся до критического уровня (частицы не появляются, пока он не
остынет) или истекла подписка. При смене «Уменьшения движения» идущий эффект перерисовывается
