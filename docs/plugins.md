...
Каждый раз, когда приложение выходит на передний план, в том числе по нажатию на уведомление,
оно проверяет, что каждый включённый плагин работает, и запускает те, что не смогли стартовать
при запуске в фоне.

Каждый запуск плагина рисует свои эффекты от своего имени. Когда один запуск сменяет другой —
перезапуск, смена аккаунта (например, по уведомлению из другого аккаунта), — обработчик `stop`
уходящего запуска не может выключить снег, который уже включил новый, а тот же самый эффект,
запрошенный новым запуском, просто передаётся ему и продолжает падать, не мигнув. Всё, что
уходящий запуск публикует в приложение после команды остановиться (кнопки, страницы, вкладки),
принимается и отбрасывается; `storage` при этом по-прежнему сохраняется.
...
Вверху экрана плагинов вместо заголовка стоит стеклянный переключатель: «Плагины» — ваши
плагины на этом устройстве, «Маркет» — плагины, которые авторы опубликовали для всех.

### Маркет

В Маркете каждый плагин показан иконкой, названием и коротким описанием, а справа кнопка
говорит, что с ним будет: «Установить», «Обновить», «Установлен» или «Ваш». «Установить» и
«Обновить» срабатывают прямо из списка, по нажатию на строку открывается страница плагина.
У плагина без иконки вместо неё своя плитка: его цвет градиентом и пазл, а не буква.

Кнопки меняются на месте одним движением: пока идёт установка, надпись уступает место
индикатору, после неё кнопка плавно перетекает в новую надпись и цвет, «Удалить» рядом с
«Обновить» выезжает сбоку, а подпись и высота панели следуют в той же анимации.

Страница открывается листом почти на весь экран, сверху остаётся полоска экрана под ним. На
ней большая иконка, название, версия и дата обновления, описание (длинное сворачивается до
пяти строк и раскрывается по «ещё»), автор и разрешения. Автор — тот, кто опубликовал плагин:
Маркет присылает только его Telegram id, а имя, @username и аватарку приложение берёт из
Telegram, если этот человек вам известен, и рисует рядом бейдж AorusGram, если он у него есть.
Нажатие открывает профиль. Каждое разрешение нарисовано значком, названием и описанием того,
что оно позволяет. Кнопка внизу всегда на месте, как бы далеко ни прокрутили страницу.

| Кнопка | Когда |
|---|---|
| Установить | Плагина на устройстве нет |
| Обновить | Установлена более старая версия. Рядом «Удалить» |
| Удалить | Установлена та же или более новая версия |
| Открыть | Это ваш плагин, он открывается в управлении из «Моих плагинов» |

Установленный из Маркета плагин появляется в «Плагинах» с названием, описанием, версией и
иконкой из Маркета и сразу выключен. При включении показываются его разрешения, как у любого
плагина. Обновление заменяет код, поэтому плагин выключается и разрешения отзываются: новый код
никогда не получает старых разрешений. Если плагин был включён, приложение сразу предложит
проверить разрешения и включить его снова. Когда в Маркете выходит новая версия
установленного плагина, в «Плагинах» под его названием пишется «Доступно обновление».

### Публикация

«Оформление» и управление в Маркете — разные экраны. «Оформление» — это то, как плагин выглядит
здесь: баннер, название, описание, версия, значок и цвет. Для ещё не опубликованного плагина
внизу кнопка «Опубликовать», для опубликованного — строка «Управление в Маркете».

Над названием строка «Баннер»: по нажатию открывается галерея, выбранное фото кадрируется в
квадрат со скруглёнными углами (двигайте и масштабируйте), и именно оно уходит в Маркет
иконкой плагина. HEIC из «Фото» перед отправкой перекодируется в JPEG.

При первой публикации приложение спрашивает идентификатор в Маркете: латиница, цифры, точка,
дефис и подчёркивание, начинается с буквы (`^[a-z][a-z0-9._-]{1,79}$`). Он закрепляется за
вами и потом не меняется. Отправляются код, название, описание и версия, а автором сервер
считает владельца лицензии этого устройства — плагин ничего о себе не сообщает. После
публикации отправляется баннер.

| Ответ | Что значит |
|---|---|
| «Опубликовано» | Версия сразу в Маркете |
| «Ваш плагин на модерации» | Версия ждёт модератора. В Маркете остаётся предыдущая одобренная, и у тех, кто установил плагин, «Обновить» появится после одобрения |
| «Плагин отклонён» | С причиной от модератора. Исправьте и отправьте снова |
| «Плагин снят с публикации» | Плагина больше нет в каталоге |

Версия — `MAJOR.MINOR.PATCH`. Одобренную версию нельзя отправить повторно: если такая уже есть,
приложение предложит поднять номер (например, 1.0.0 до 1.0.1) и опубликовать. Версию на
модерации или отклонённую можно отправить заново с тем же номером. Обновление всегда проходит
модерацию. «Публикация недоступна: автор заблокирован» — блокировка автора, при ней нельзя ни
публиковать, ни генерировать, ни загружать баннер.

### Мои плагины

Когда у вас есть опубликованные плагины, в Маркете над каталогом появляется строка «Мои
плагины». В ней каждый плагин с состоянием его версий: опубликована, на модерации, отклонена с
причиной или снята. Нажатие открывает экран управления плагином:

| Блок | Что в нём |
|---|---|
| Маркет | Где стоит каждая версия: опубликована, на модерации, отклонена с причиной, снята |
| Плагин | Баннер, название, описание, версия и «Редактор» с кодом — то, что уйдёт со следующей публикацией |
| Опубликовать | Отправляет новую версию |
| Удалить плагин | Убирает из Маркета все ваши версии этого id вместе с кодом и иконкой |

Значка и цвета здесь нет — это оформление, а не публикация. Если на этом устройстве копии нет,
приложение делает её из опубликованной версии.

«Удалить плагин» спрашивает подтверждение, а после ответа сервера пишет «Плагин удалён». Плагин
пропадает из каталога и из «Моих плагинов», его иконка и код больше не скачиваются, а этот id
снова свободен — его можно опубликовать заново как первую публикацию. Копия на этом телефоне
остаётся обычным плагином, уже не связанным с Маркетом; у тех, кто установил плагин раньше, их
копии тоже остаются. Удалить можно только свой плагин: на чужих карточках этой кнопки нет. Когда
удалён последний, «Мои плагины» закрываются сами.

### AorusAI в редакторе

В редакторе кода справа вверху три кнопки: «Сохранить», «AI» и «⋯». «AI» открывает поле
запроса — лист высотой ровно со своё содержимое, который поднимается вместе с клавиатурой одним
движением и больше не прыгает. Опишите, что должен делать плагин, и отправьте.

Каждая генерация — отдельный плагин: код из редактора не отправляется, чтобы его не слили с
новым. Новый код печатается поверх старого: старый тускнеет и уходит сверху вниз по мере того,
как пишется новый, и к концу от него ничего не остаётся. Ничего не сохраняется, пока вы не
нажмёте «Сохранить». Если у плагина ещё стандартное название, он получает название и описание
из ответа. Запрос — от 8 до 4000 символов. Если автор заблокирован, генерация недоступна.

### История правок

Справа внизу редактора, над панелью инструментов, а при открытой клавиатуре — над ней, стоят
две стрелки на стекле: назад и вперёд по правкам. Стрелка горит цветом акцента, когда есть куда
идти, и тускнеет, когда некуда. Набор объединяется в один шаг до паузы, новой строки или
вставки; генерация AorusAI — тоже один шаг, так что стрелка назад возвращает прежний код
целиком. «Сохранить» историю не сбрасывает.
