...
def _patch_avatar_tint_publish(tg: Path) -> None:
    ...
    if "publishAvatarTint" in text:
        ...
        text = _replace_once(
            ...
            "        // Narrow on purpose, to exactly the case this is for: a peer with no\n"
            "        // photograph who does have a profile background. `smallProfileImage` answers\n"
            "        // the first question immediately, where `galleryEntries` is empty for a\n"
            "        // moment while a real photo loads; both are required so that moment cannot\n"
            "        // repaint anything. `effectiveProfileColor` answers the second -- without one\n"
            "        // the cover draws nothing of its own, and such a profile keeps the page it\n"
            "        // has always had.\n"
            "        // A header that is a profile background rather than a photograph.\n"
            "        //\n"
            "        // Not sampled, deliberately. Sampling exists because a photograph has no\n"
            "        // palette entry -- its colour can only be learnt by rendering it. A profile\n"
            "        // background has one: the cover is handed two colours and draws its gradient\n"
            "        // between them, so the colour its bottom edge ends on is known outright.\n"
            "        // Reading it back off the screen only added ways to fail, and each was hit in\n"
            "        // turn: a component view with no size, a cover filled with `.clear`, a band\n"
            "        // clamped to a third of a point, a banner faded out mid-scroll.\n"
            "        //\n"
            "        // Which colour it is comes from the same branch that chooses the cover\'s\n"
            "        // subject a few hundred lines up, so the two cannot disagree. A star-gift\n"
            "        // status makes the cover a `.status`, drawn as a radial gradient from the\n"
            "        // avatar outwards between the gift\'s inner and outer colours -- the bottom\n"
            "        // edge is the far end of it, which is the outer one. Anything else is a\n"
            "        // `.peer`, drawn as an axial gradient with `startPoint` at the bottom and the\n"
            "        // profile palette\'s main colour first, so the bottom edge is that main colour.\n"
            "        if listContainerNode.galleryEntries.isEmpty, peer.smallProfileImage == nil {\n"
            "            var aorusBottomEdgeColor: UIColor?\n"
            "            if let status = peer.emojiStatus, case let .starGift(_, _, _, _, _, _, outerColor, _, _) = status.content {\n"
            "                aorusBottomEdgeColor = UIColor(rgb: UInt32(bitPattern: outerColor))\n"
            "            } else if let profileColor = peer.profileColor, let presentationData = self.presentationData {\n"
