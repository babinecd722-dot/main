...
def patch_incoming_message_hook(tg: Path) -> None:
    ...
    prefilter_code = (
        ...
        "                    // Only ever filter one-on-one private chats with NON-contacts. Groups and\n"
        "                    // channels legitimately carry finance/crypto/link chatter, and people you\n"
        "                    // already know must never lose messages \u2014 unsolicited spam is a DM-from-\n"
        "                    // a-stranger problem, so scope the filter there.\n"
        "                    guard mid.peerId.namespace == Namespaces.Peer.CloudUser else { return true }\n"
        "                    if transaction.isPeerContact(peerId: mid.peerId) { return true }\n"
