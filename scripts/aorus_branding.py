...
def patch_deleted_message_quote_reply(tg: Path) -> None:
    """Reply to a preserved deleted message as an inline blockquote on send.
    ...
    if "AorusGram: deleted-message quote reply on send" in t:
    ...
    if anchor not in t:
        print("DeletedQuoteReply: sendMessages anchor not found — skipped")
        return
    inject = (
        "            var messages = messages\n"
        "            var shouldOpenScheduledMessages = false\n"
        ...
        "                var aorusQuoted = String(aorusDeleted.text.dropLast(2))\n"
    ...
    print("DeletedQuoteReply: injected blockquote-on-send for deleted-message replies")
...
def patch_app_delegate_anti_spam_toast(tg: Path) -> None:
    ...
    hook = (
        ...
        "            }\n"
        "            let aorusSpamIcon = (UIImage(systemName: \"hand.raised.slash.fill\", withConfiguration: UIImage.SymbolConfiguration(pointSize: 30.0, weight: .medium))?.withTintColor(UIColor.white, renderingMode: .alwaysOriginal)) ?? UIImage()\n"
        "            if let controller = app.rootController.viewControllers.last as? ViewController {\n"
        "                controller.present(\n"
        "                    UndoOverlayController(\n"
        "                    presentationData: presentationData,\n"
        "                    content: .image(image: aorusSpamIcon, title: isRu ? \"Антиспам\" : \"Anti-spam\", text: text, round: false, undoText: nil),\n"
