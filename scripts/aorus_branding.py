...
def patch_deleted_message_quote_reply(tg: Path) -> None:
    """Reply to a preserved deleted message reliably, keeping the native reply bar.
    ...
    peer (and, worse, sending it makes the message flash and vanish). The reliable design:

      1. When the user taps Reply (menu or swipe) on such a message we DO set up the
         native "In reply to X" bar, AND we insert the deleted text into the composer as
         a real collapsible blockquote using ChatTextInputAttributes.block. The blockquote
         entity is then produced by the app's own, battle-tested send pipeline — never
         hand-crafted — so the message sends normally and the quote is visible to everyone.
      2. At send time (ChatController.sendMessages) we simply DROP the server reply when
         its target is a preserved deleted message. Nothing else is rewritten. This avoids
         a doomed server reply to a message that no longer exists on the server.

    Replies to normal (non-deleted) messages are completely untouched.
    """
    # --- 1) Context menu "Reply" action ---
        ...
        if "AorusGram: deleted-message quote reply" in t:
            ...
            if anchor not in t:
                ...
                replacement = (
                    ...
                    "                let aorusReplyDeleted = aorusReplyMsg.text.hasSuffix(\"\\u{2063}\\u{2064}\")\n"
                    "                interfaceInteraction.setupReplyMessage(messages[0].id, nil, { transition, completed in\n"
                    "                    c?.dismiss(result: .custom(transition), completion: {\n"
                    "                        completed()\n"
                    "                    })\n"
                    "                })\n"
                    "                if aorusReplyDeleted {\n"
                    "                    // The server has no copy of this message, so also drop the deleted\n"
                    "                    // text into the composer as a native collapsible blockquote — the app\n"
                    "                    // builds a correct entity and the quote reaches everyone. The server\n"
                    "                    // reply itself is dropped at send time.\n"
                    ...
                    "                        if current.inputText.length > 0 { return (current, inputMode) }\n"
                ...
                path.write_text(t, encoding="utf-8")
                print("DeletedQuoteReply: injected native reply + composer blockquote (menu)")
    else:
        print("DeletedQuoteReply: ChatInterfaceStateContextMenus.swift not found, skip menu")

    # --- 2) Swipe-to-reply + 3) drop server reply at send, in ChatController ---
    ...
    # 2) Swipe: same native-bar + composer-blockquote behaviour.
    if "AorusGram: deleted-message quote reply (swipe" in t2:
        print("DeletedQuoteReply: swipe already injected")
    else:
        swipe_anchor = (
    ...
    # 3) At send: drop the server reply when its target is a preserved deleted message.
    if "AorusGram: drop server reply to deleted" not in t2:
        send_anchor = (
        ...
        if send_anchor in t2:
            send_inject = (
                "            var messages = messages\n"
                "            var shouldOpenScheduledMessages = false\n"
                "            // AorusGram: drop server reply to deleted — the target no longer exists on\n"
                "            // the server, so a server reply would flash-and-vanish. The quote is already\n"
                "            // carried inline as a native blockquote in the composer text.\n"
                "            messages = messages.map { aorusMessage -> EnqueueMessage in\n"
                "                guard case let .message(_, _, _, _, _, aorusReplyToMessageId, _, _, _, _) = aorusMessage else { return aorusMessage }\n"
                "                guard let aorusReply = aorusReplyToMessageId, let aorusDeleted = self.chatDisplayNode.historyNode.messageInCurrentHistoryView(aorusReply.messageId), aorusDeleted.text.hasSuffix(\"\\u{2063}\\u{2064}\") else { return aorusMessage }\n"
                "                return aorusMessage.withUpdatedReplyToMessageId(nil)\n"
                "            }\n"
            )
            t2 = t2.replace(send_anchor, send_inject, 1)
        else:
            print("DeletedQuoteReply: sendMessages anchor not found — skipped drop-reply")

    cc.write_text(t2, encoding="utf-8")
    print("DeletedQuoteReply: injected swipe blockquote + drop-server-reply")
...
def patch_incoming_message_hook(tg: Path) -> None:
    ...
    prefilter_code = (
        ...
        "                        if let mapped = aorusDeobMap[ch] { result.append(mapped) }\n"
        "                        else if (ch >= \"a\" && ch <= \"z\") || (ch >= \"0\" && ch <= \"9\") { result.append(ch) }\n"
        "                    }\n"
        "                    return result\n"
        "                }\n"
        ...
        "                }\n"
        "                func aorusPostSpamNotice(_ noticePeerId: Int64, _ noticeMsgId: Int32, _ noticeReason: String) {\n"
        "                    DispatchQueue.main.async {\n"
        "                        NotificationCenter.default.post(name: NSNotification.Name(\"aorusgram_spam_detected\"), object: nil, userInfo: [\"peerId\": noticePeerId, \"msgId\": NSNumber(value: noticeMsgId), \"reason\": noticeReason])\n"
        "                    }\n"
        "                }\n"
        "                // Threats are NEVER hidden — the user must see them; they only raise an alert.\n"
        "                func aorusThreatReason(_ rawText: String) -> String? {\n"
        "                    guard aorusThreatProtection else { return nil }\n"
        "                    let text = aorusNormalizeSpamText(rawText)\n"
        "                    if text.isEmpty { return nil }\n"
        "                    let compacted = aorusCompactSpamText(text)\n"
        "                    for p in aorusThreatPatterns where aorusSpamContains(p, normalized: text, compacted: compacted) { return p }\n"
        "                    let deob = aorusDeobfuscate(rawText)\n"
        "                    if !deob.isEmpty { for tok in aorusThreatTokensLatin where !tok.isEmpty && deob.contains(tok) { return tok } }\n"
        "                    return nil\n"
        "                }\n"
        "                // Spam / stop-words / blocked → hide. Returns a reason when hidden, else nil.\n"
        "                func aorusSpamHideReason(_ identity: Int64, _ rawText: String) -> String? {\n"
        "                    if identity == 777000 || aorusAllowedPeerIds.contains(identity) { return nil }\n"
        "                    if aorusAutoBlockOn && aorusBlockedPeerIds.contains(identity) { return \"blockedUser\" }\n"
        "                    let text = aorusNormalizeSpamText(rawText)\n"
        "                    if text.isEmpty { return nil }\n"
        "                    let compacted = aorusCompactSpamText(text)\n"
        "                    if aorusStopWordsOn {\n"
        ...
        "                            if !cleanKeyword.isEmpty && aorusSpamContains(cleanKeyword, normalized: text, compacted: compacted) { return \"keyword\" }\n"
        "                        }\n"
        "                    }\n"
        ...
        "                    let contactBait = [\"пиши в лс\", \"пиши в личку\", \"напиши мне\", \"подробности в лс\", \"за деталями\", \"жду в лс\", \"переходи по ссылке\", \"подпишись на канал\", \"пиши в телеграм\", \"write me\", \"dm me\", \"pm me\", \"message me\", \"contact me\", \"text me\", \"click the link\", \"join my channel\", \"add me\", \"hit me up\"]\n"
        "                    if contactBait.contains(where: { aorusSpamContains($0, normalized: text, compacted: compacted) }) { score += links > 0 ? 2 : 1 }\n"
        "                    let moneySignals = [\"₽\", \"$\", \"€\", \"£\", \"руб\", \"доллар\", \"евро\", \"оплата\", \"выплата\", \"прибыль\", \"процент\", \"x2\", \"x3\", \"x10\", \"usd\", \"eur\", \"payout\", \"cashout\"]\n"
        "                    if moneySignals.contains(where: { text.contains($0) }) { score += 1 }\n"
        "                    let phishingLinkHints = [\"login\", \"verify\", \"wallet\", \"airdrop\", \"bonus\", \"gift\", \"support\", \"security\", \"account\", \"confirm\", \"claim\", \"unlock\", \"restore\", \"recover\", \"billing\", \"payment\", \"update\", \"signin\", \"auth\"]\n"
        "                    if links > 0 && phishingLinkHints.contains(where: { text.contains($0) }) { score += 2 }\n"
        "                    if text.count < 24 && links == 0 { return nil }\n"
        "                    return score >= 3 ? \"spam\" : nil\n"
        "                }\n"
        "                // Smart clean-up of incoming text: shout-case (ALL CAPS) is folded back to\n"
        "                // normal case and the first letter is capitalised. Skipped for anything with a\n"
        "                // link / mention / hashtag / command / code, whose casing is meaningful.\n"
        "                func aorusSmartNormalize(_ rawText: String) -> String {\n"
        "                    if rawText.count < 2 { return rawText }\n"
        "                    if rawText.contains(\"http\") || rawText.contains(\"@\") || rawText.contains(\"#\") || rawText.contains(\"/\") || rawText.contains(\"`\") { return rawText }\n"
        "                    var result = rawText\n"
        "                    let letters = result.filter { $0.isLetter }\n"
        "                    if letters.count >= 5 {\n"
        "                        let uppers = letters.reduce(0) { $0 + ($1.isUppercase ? 1 : 0) }\n"
        "                        if Double(uppers) / Double(letters.count) > 0.6 { result = result.lowercased() }\n"
        "                    }\n"
        "                    if let idx = result.firstIndex(where: { $0.isLetter }), result[idx].isLowercase {\n"
        "                        result.replaceSubrange(idx...idx, with: String(result[idx]).uppercased())\n"
        "                    }\n"
        "                    return result\n"
        "                }\n"
        "                func aorusRebuildStoreMessage(_ storeMsg: StoreMessage, text: String) -> StoreMessage {\n"
        "                    return StoreMessage(id: storeMsg.id, customStableId: storeMsg.customStableId, globallyUniqueId: storeMsg.globallyUniqueId, groupingKey: storeMsg.groupingKey, threadId: storeMsg.threadId, timestamp: storeMsg.timestamp, flags: storeMsg.flags, tags: storeMsg.tags, globalTags: storeMsg.globalTags, localTags: storeMsg.localTags, forwardInfo: storeMsg.forwardInfo, authorId: storeMsg.authorId, text: text, attributes: storeMsg.attributes, media: storeMsg.media)\n"
        "                }\n"
        "                let aorusVisibleMessages: [StoreMessage] = aorusAntiSpamEnabled ? messages.compactMap { storeMsg -> StoreMessage? in\n"
        "                    guard storeMsg.flags.contains(.Incoming) else { return storeMsg }\n"
        "                    guard case let .Id(mid) = storeMsg.id else { return storeMsg }\n"
        "                    // Spam hiding: only one-on-one private chats with NON-contacts. Groups and\n"
        ...
        "                        // Threat → alert only (message stays visible). Spam → hide + alert.\n"
        ...
        "                            aorusPostSpamNotice(identity, mid.id, hideReason)\n"
        "                            return nil\n"
        "                        }\n"
        "                    }\n"
        "                    // Text clean-up applies to every incoming message (not just DMs).\n"
        "                    if aorusTextCleanupOn {\n"
        "                        let aorusNorm = aorusSmartNormalize(storeMsg.text)\n"
        "                        if aorusNorm != storeMsg.text { return aorusRebuildStoreMessage(storeMsg, text: aorusNorm) }\n"
        "                    }\n"
        "                    return storeMsg\n"
        "                } : messages\n"
    )
...
def patch_app_delegate_anti_spam_toast(tg: Path) -> None:
    ...
    hook = (
        ...
        "                        // Auto-report: send when the alert times out (.commit); the button is\n"
        "                        // \"Cancel\" (.undo) which stops it. Manual: the \"Report\" button (.undo) sends.\n"
        "                        let aorusShouldSend: Bool\n"
        "                        if aorusAutoReport {\n"
        "                            if case .commit = action { aorusShouldSend = true } else { aorusShouldSend = false }\n"
        "                        } else {\n"
        "                            if case .undo = action { aorusShouldSend = true } else { aorusShouldSend = false }\n"
        "                        }\n"
        "                        if aorusShouldSend, isThreat, aorusPeerId != 0, let app = self?.contextValue {\n"
...
def patch_anti_spoof_delete_preflight(tg: Path) -> None:
    ...
    if sentinel in t:
        ...
        upgraded = upgraded.replace(
            ...
            "            return !msg.flags.contains(.Incoming) && !msg.text.isEmpty && !msg.text.hasPrefix(\"Сообщение удалено\")\n",
        )
        upgraded = upgraded.replace(
            "            let text = msg.text.trimmingCharacters(in: .whitespacesAndNewlines)\n"
