...
def patch_authorization_login_title_gold(tg: Path) -> None:
    """No-op: the phone-number welcome title now keeps Telegram's default color
    (theme.list.itemPrimaryTextColor). The earlier gold tint on "Your Phone" is
    intentionally removed at the user's request — leaving the upstream source
    untouched restores the default title color."""
    print("Auth phone entry: default title color (gold tint removed)")


def patch_accent_color_purple(tg: Path) -> None:
    """Recolor the app accent from Telegram blue to a neon violet that matches the
    AorusGram icon.
    ...
    replacements = [
        ...
        ("submodules/TelegramPresentationData/Sources/DefaultDayPresentationTheme.swift",
         ...
         "startButtonColor: UIColor(rgb: 0x9b4dff),",
         "day intro button"),
    ...
    for rel, old, new, label in replacements:
        p = tg / rel
        if not p.is_file():
            print(f"AccentPurple: {rel} not found — skip {label}")
            continue
        t = p.read_text(encoding="utf-8")
        if new in t:
            print(f"AccentPurple: {label} already violet")
            continue
        if old not in t:
            print(f"AccentPurple: WARNING anchor for {label} not found — skipped")
            continue
...
def patch_incoming_message_hook(tg: Path) -> None:
    ...
    prefilter_code = (
        ...
        "                let aorusImmunePeerIds = Set((UserDefaults.standard.array(forKey: \"aorusgram_antispam_immune_peer_ids\") as? [NSNumber] ?? []).map { $0.int64Value })\n"
        ...
        "                    return result\n"
        "                }\n"
        "                func aorusCyrThreatFold(_ ch: Character) -> String? {\n"
        "                    switch ch {\n"
        "                    case \"0\", \"o\", \"о\": return \"о\"\n"
        "                    case \"1\", \"i\", \"і\": return \"и\"\n"
        "                    case \"3\", \"e\", \"е\": return \"е\"\n"
        ...
        "                    }\n"
        "                }\n"
        "                func aorusCyrThreatReason(_ normalizedText: String) -> String? {\n"
        "                    var words: [String] = []\n"
        "                    var current = \"\"\n"
        "                    func flush() { if !current.isEmpty { words.append(current); current.removeAll(keepingCapacity: true) } }\n"
        "                    for ch in normalizedText { if let folded = aorusCyrThreatFold(ch) { current += folded } else { flush() } }\n"
        "                    flush()\n"
        ...
        "                            if aorusThreatTokensCyr.contains(combined) { return combined }\n"
        "                            if aorusThreatPrefixesCyr.contains(where: { combined.hasPrefix($0) }) { return combined }\n"
        "                        }\n"
        "                    }\n"
        "                    return nil\n"
        "                }\n"
        ...
        "                    if let cyrReason = aorusCyrThreatReason(text) { return cyrReason }\n"
        ...
        "                    // Blocked / allowed / service peers are decided by the caller.\n"
        "                    let text = aorusNormalizeSpamText(rawText)\n"
        "                    if text.isEmpty { return nil }\n"
        "                    let compacted = aorusCompactSpamText(text)\n"
        "                    if aorusStopWordsOn {\n"
        ...
        "                        // A peer the user manually un-blocked is immune: no auto-block, no spam/\n"
        "                        // blocked hiding, and NO spam/blocked notice. Threats still alert though —\n"
        "                        // immunity is not a shield against doxx/swat/flood, only against the spam\n"
        "                        // + blocked path.\n"
        "                        let aorusIsImmunePeer = aorusImmunePeerIds.contains(peerIdentity) || (authorIdentity != nil && aorusImmunePeerIds.contains(authorIdentity!))\n"
        ...
        "                            } else if aorusIsImmunePeer {\n"
        "                                // immune: fall through, message stays visible, no notice\n"
