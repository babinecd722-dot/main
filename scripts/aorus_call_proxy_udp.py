...
def patch_tgcalls_reflector_socks5_udp(tg: Path) -> None:
    ...
    if socket_class_marker in text and socket_schema_marker not in text:
        ...
        udp_replacement = (
            "            // AorusGram: the SOCKS5 relay is the in-process VLESS core, so the\n"
            "            // proxy address is loopback. A UDP socket bound to the interface\n"
            "            // address cannot reach 127.0.0.1: the route for loopback leaves via\n"
            "            // lo0, where the interface address is not a valid source, so every\n"
            "            // sendto() fails while the TCP control channel still connects — a\n"
            "            // call that negotiates and then carries no media. Bind to the proxy's\n"
            "            // own address in that case. A loopback-bound port is explicitly\n"
            "            // allowed further down (\"Still allowing it since it's localhost\").\n"
            "            const bool aorusUsesSocks5 = proxy().type == rtc::ProxyType::PROXY_SOCKS5;\n"
            "            const bool aorusProxyIsLocal = aorusUsesSocks5 && proxy().address.IsLoopbackIP();\n"
            "            const rtc::SocketAddress aorusUdpBindAddress = aorusProxyIsLocal\n"
            "                ? rtc::SocketAddress(proxy().address.ipaddr(), 0)\n"
            "                : rtc::SocketAddress(Network()->GetBestIP(), 0);\n"
            "            rtc::AsyncPacketSocket* aorusUdpSocket = socket_factory()->CreateUdpSocket(aorusUdpBindAddress, min_port(), max_port());\n"
            "            if (aorusUdpSocket && aorusUsesSocks5) {\n"
