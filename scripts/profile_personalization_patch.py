...
def _patch_profile_list_glass(tg: Path) -> None:
    ...
    screen = _replace_once(
        ...
        "        }\n"
        "        if backdrop == nil {\n"
        "            // The page's own view, so it follows a swipe between photos with the header's fade\n"
        "            // and every pane rather than cutting to the next photo's page after it lands.\n"
        "            let imageView = AorusProfilePageImageView(peerId: self.peerId.id._internalGetInt64Value())\n"
