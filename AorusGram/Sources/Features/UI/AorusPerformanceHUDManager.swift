...
private struct AorusPerformanceHUDL10n {
    ...
    let performanceBattery: String
    let performanceNetwork: String
    let performanceDisk: String
    let performanceThermal: String
    let thermalNominal: String
    let thermalFair: String
    let thermalSerious: String
    ...
    static var current: AorusPerformanceHUDL10n {
        ...
        if code.hasPrefix("ru") {
            return AorusPerformanceHUDL10n(
                ...
                performanceCPU: "CPU",
                performanceFPS: "FPS",
                performanceBattery: "Батарея",
                performanceNetwork: "Сеть",
                performanceDisk: "Диск",
                performanceThermal: "Нагрев",
                thermalNominal: "НОРМ",
                thermalFair: "ТЕПЛО",
                thermalSerious: "ГОРЯЧО",
                thermalCritical: "КРИТ"
            )
        }
        return AorusPerformanceHUDL10n(
            performanceRAM: "RAM",
            performanceCPU: "CPU",
            performanceFPS: "FPS",
            performanceBattery: "Battery",
            performanceNetwork: "Network",
            performanceDisk: "Disk",
            performanceThermal: "Thermal",
            thermalNominal: "OK",
            thermalFair: "WARM",
            thermalSerious: "HOT",
...
private final class AorusPerformanceHUDView: UIView {
    ...
    private func thermalText(_ state: ProcessInfo.ThermalState, l10n: AorusPerformanceHUDL10n) -> (text: String, style: RowStyle) {
        switch state {
        case .nominal:
    ...
    private func _refresh() {
        if !observing {
            ...
            NotificationCenter.default.addObserver(self, selector: #selector(onSettingsChanged),
                                                   name: NSNotification.Name("aorusgram_settings_changed"), object: nil)
